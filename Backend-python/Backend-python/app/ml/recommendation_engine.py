...
class RecommendationEngine:
    def __init__(self):
        ...
        self.top_drivers = loader.load_global_drivers()
        self.recommendation_rules = loader.load_recommendation_rules()
        self.predictor = Predictor()

    def build_X_for_inference(self, raw_df: pd.DataFrame) -> pd.DataFrame:
        return self.predictor.prepare_features(raw_df)

    def predict_with_contribs(self, X_aligned: pd.DataFrame):
        dmat = xgb.DMatrix(X_aligned, feature_names=self.feature_cols)
    ...
    def dynamic_fallback_action(self, feature_name: str) -> str:
        ...
        if "pump" in f:
            return f"Check pump availability and maintenance affecting {clean_name}."
        if "rating" in f:
    ...
    def analyze(self, stations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if "station_id" in raw_df.columns:
            raw_df["station_id"] = raw_df["station_id"].astype(str).str.strip()
            raw_df = raw_df[raw_df["station_id"].str.lower() != "station_id"]
        ...
        for i in range(len(raw_df)):
            ...
            results.append({
                "station_id": str(raw_df["station_id"].iloc[i]) if "station_id" in raw_df.columns else None,
