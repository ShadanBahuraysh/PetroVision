...
ERROR_DETAIL = "Unexpected analysis service error"
...
def test_run_all_analysis_failure(monkeypatch):
    def fake_run_full_analysis(force=False):
...
def test_overview_failure(monkeypatch):
    def fake_get_overview():
        raise Exception("Overview failed")
...
def test_top_bottom_failure(monkeypatch):
    def fake_get_top_bottom_10():
        raise Exception("Top bottom failed")
...
def test_station_analysis_failure(monkeypatch):
    def fake_get_station_analysis(station_id):
        raise Exception("Station analysis failed")
...
def test_explain_overview_failure(monkeypatch):
    def fake_explain_overview():
        raise Exception("Overview explanation failed")
...
def test_export_report_success(monkeypatch):
    def fake_build_export_rows():
        return (
            ...
            [{"station_id": "S001", "score": 80, "status": "Good", "priority": "Low", "top_issue": "No major issue", "recommendation": "Monitor station performance."}],
        )
...
def test_export_report_no_data_error(monkeypatch):
    from fastapi import HTTPException

    def fake_build_export_rows():
        raise HTTPException(status_code=400, detail="No analysis data available. Please run analysis first.")
    ...
    assert response.json()["detail"] == "No analysis data available. Please run analysis first."
...
def test_download_csv_success(monkeypatch):
    def fake_build_export_rows():
        return (
            ...
            ["Insight"],
            [{"station_id": "S001", "score": 88, "status": "Good", "priority": "Low", "top_issue": "No issue", "recommendation": "Monitor station"}],
        )
...
def test_download_invalid_type(monkeypatch):
    ...
    assert response.status_code == 400
    assert response.json()["detail"] == "Unsupported file type. Use csv or excel."
